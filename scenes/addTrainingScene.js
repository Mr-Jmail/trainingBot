...
module.exports = new Scenes.WizardScene("addTrainingScene", 
    ctx => {
        ...
        ctx.reply("Введите название новой тренировки", {reply_markup: {inline_keyboard: [[{text: "Назад", callback_data: "backToManageTrainingsScene"}]]}})
        return ctx.wizard.next()
    },
    async ctx => {
        if(ctx.callbackQuery?.data == "backToManageTrainingsScene") return ctx.scene.enter("manageTrainingsScene")
        if(!await trainingsTittleIsFree(ctx.message.text)) return await ctx.reply(`Название "${ctx.message.text}" уже занято другой тренировкой. Введите другое`, {reply_markup: {inline_keyboard: [[{text: "Назад", callback_data: "backToManageTrainingsScene"}]]}})
    ...
    async ctx => {
        ...
        if(ctx.callbackQuery?.data == "accepted") {
            ...
            await ctx.reply("Отлично, добавил тренировку", {reply_markup: {inline_keyboard: [[{text: "Назад", callback_data: "toSceneStart"}]]}})
        }
        if(ctx.callbackQuery?.data == "declined") {
            return await ctx.reply("Что хотите сделать?", {reply_markup: {inline_keyboard: [[{text: "Начать добавление тренировки заново", callback_data: "toSceneStart"}], [{text: "Отменить добавление тренировки", callback_data: "cancelAdding"}], [{text: "Назад", callback_data: "stopTrainingRecording"}]]}})
        }
        if(ctx.callbackQuery?.data == "cancelAdding") {
            await ctx.reply("Добавление тренировки отменено, перенаправляю в основное меню")
            ...
            await ctx.reply("Вот так тренировка будет отображаться пользователю:")
            await sendMessages(ctx.scene.session.state.messages, ctx).catch(err => console.log(err))
            return await ctx.reply("Все отлично, подтверждаю добавление тренировки?", {reply_markup: {inline_keyboard: [[{text: "Да", callback_data: "accepted"}, {text: "Нет", callback_data: "declined"}]]}})
        }
